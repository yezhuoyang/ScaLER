...
class TestQEPGGraphShape:
    ...
    def test_graph_shape_matches_python(self, circuit_base_path, circuit_name):
        ...
        # Build C++ QEPG (use the original normalized string, not
        # str(stim_circuit) which uses stim's batched format that the
        # C++ parser cannot handle)
        cpp_graph = compile_QEPG(stim_str)
        ...
        cpp_matrix = np.array(return_detector_matrix(stim_str))
        ...
        print(f"{'=' * 60}")
        print(
            f"Python: noise={py_num_noise}, detectors={py_num_detectors}, matrix={py_matrix_shape}"
        )
        print(
            f"C++:    noise={cpp_num_noise}, detectors={cpp_num_detectors}, matrix={cpp_matrix_shape}"
        )
    ...
    def test_detector_matrix_values_match(self, circuit_base_path, circuit_name):
        ...
        # Build C++ QEPG (use normalized string, not batched stim format)
        cpp_matrix = np.array(return_detector_matrix(stim_str))
...
class TestQEPGSamplerCorrectness:
    ...
    def test_sampler_vs_stim_random_errors(self, circuit_base_path, circuit_name):
        ...
        # Get QEPG detector matrix (use normalized string, not batched stim format)
        detector_matrix = np.array(return_detector_matrix(stim_str))
        ...
        for weight in range(1, max_weight + 1):
            for trial in range(num_trials):
                ...
                stim_result = transpile_stim_with_noise_vector(
                    stim_str, list(noise_vector), total_noise
                )
    ...
    def test_single_error_all_positions(self, circuit_base_path, circuit_name):
        ...
        # Get QEPG detector matrix (use normalized string)
        detector_matrix = np.array(return_detector_matrix(stim_str))
        ...
        for noise_idx in range(total_noise):
            for error_type_idx, error_type in enumerate(error_types):
                ...
                if stim_result != qepg_result:
                    ...
                    pytest.fail(
                        f"Mismatch for {circuit_name}, {error_type} at pos {noise_idx}"
                    )
